...
extension VepayPaymentController {
    ...
    func updateArrowToSelectedCard(animated: Bool = true, scrollToItem: Bool = true) {
        ...
        } else if cellFrame.maxX > cardView.frame.maxX + 32 {
            center = cardView.frame.maxX + 32 - cellFrame.width / 2
        } else {
        ...
        animated ? UIView.animate(withDuration: 0.24, animations: animation) : animation()
    }
